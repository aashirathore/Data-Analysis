...
public class ShowAnalysisByAge extends JFrame implements ActionListener{
    ...
    setResizable(false);
	 ...
	 void addcontrols() {
		...
		final ShowGraphAge gpCount = new ShowGraphAge(dbaList,1," Age vs Gender Count", "Customers Count");
		final ShowGraphAge gpAI = new ShowGraphAge(dbaList,2," Age vs Total Annual Income", "Annual Income ");
		final ShowGraphAge gpSS = new ShowGraphAge(dbaList,3," Age vs Average Spending Score", "Avg Spending Score");
        pnlGrid.add(gpCount);
        pnlGrid.add(gpAI);
        pnlGrid.add(gpSS);
